...
void zendPageBasis(AsyncWebServerRequest *request) {
  ...
  page += F("\" required><span class=\"help\"><strong>Do not change this unless you know exactly what you are doing.</strong> Changing the ECU_ID requires re-pairing all inverters. It also changes the installation identity used in MQTT, Home Assistant and other reporting systems, which can create new devices and split reporting history. Keep the current value for normal use; restore the original settings backup when replacing an ECU.</span></div>");

...
<option value="3" invtype_3>QT2 (experimental monitoring)</option>
</select>
...
<span class="help">Disable an input only when no panel is connected. DS3 and YC600 expose two inputs; QS1 and QT2 expose four. QT2 has two shared DC-voltage readings and supports monitoring only.</span>
</div>
</div>
)=====";
...
function myFunction(){
 if(["1", "3"].includes(document.getElementById("sel").value) ) {
    showFunction();
 } else {
   hideFunction();
 }
